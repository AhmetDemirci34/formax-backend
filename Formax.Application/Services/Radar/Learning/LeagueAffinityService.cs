...
{
    /// <summary>
    /// Radar Learning (R.14.4) — default orchestration. Does the I/O (gets the interest
    /// profile via the R.14.2 service), then calls the pure engine. No persistence.
    /// </summary>
    public sealed class LeagueAffinityService : ILeagueAffinityService
    {
        private readonly IUserInterestProfileService _profileService;
        private readonly ILeagueAffinityEngine _engine;

        public LeagueAffinityService(
            IUserInterestProfileService profileService,
            ILeagueAffinityEngine engine)
        {
            _profileService = profileService;
            _engine = engine;
        }

        public async Task<LeagueAffinityDto> GetAsync(int userId, CancellationToken ct = default)
        {
